...
import numpy as np
import time
from deeppipe_api.utils import (
    get_pipelines_settings_on_dataset,
    get_response,
    generate_settings,
    load_data,
    get_scores,
...
class DeepPipe:
    def __init__(
        ...
        with open(
            os.path.join(rootdir, "experiments", "data", "classification.json")
        ) as f:
            configs = json.load(f)
        with open(
            os.path.join(
                rootdir, "experiments", "data", "tensor_oboe_meta_dataset.json"
            )
        ) as f:
            data = json.load(f)
        with open(
            os.path.join(rootdir, "experiments", "configurations", "DeepPipeTOboe.json")
        ) as f:
        ...
        self.list_for_selector = torch.LongTensor(data["list_for_selector"]).to(
            self.device
        )
        ...
        self.checkpoint_path = os.path.join(
            rootdir, "experiments", "checkpoints", experiment_id
        )
    ...
    def fit(self, X, y):
        ...
        pipelines_settings = self.get_pipelines_settings_on_dataset(
            self.settings, num_points_tr, num_features_tr
        )

        # split train and test
        X_train, X_val, y_train, y_val = train_test_split(X, y, test_size=0.2)
        categorical = np.array(X_train.dtypes == "category")
        output = get_matrix_and_masks(
            self.hidden_dim,
            self.original_pipelines,
            self.list_for_concatenator,
            self.algorithm_domain,
            self.list_for_selector,
            self.encoder_layers,
        ...
        if np.sum(categorical) > 0:
            ...
            for i, setting in enumerate(pipelines_settings):
                if (
                    setting["imputer"]["hyperparameters"]["strategy"]
                    in ["most_frequent", "constant"]
                ) and (setting["encoder"]["algorithm"] is not None):
        ...
        for pipeline_id in original_x:
            try:
                if on_time:
                    ...
                    temp_response, _ = get_response(
                        data, new_idx, valid_pipelines, categorical, self.apply_cv
                    )
                    ...
                    info_tuple = (
                        spent_time,
                        temp_response,
                        torch.max(response).detach().cpu().numpy(),
                        new_idx,
                    )
        ...
        if on_time:
            ...
            for bo_iter in pbar:
                ...
                optimizer = torch.optim.Adam(
                    [
                        {"params": model.model.parameters(), "lr": self.lr},
                ...
                if self.create_ensemble:
                    _, temp_model = get_response(
                        data, pipeline_id, valid_pipelines, categorical, apply_cv=False
                    )
                    fitted_pipelines.append(temp_model)
                temp_y, _ = get_response(
                    data,
                    pipeline_id,
                    valid_pipelines,
                    categorical,
                    apply_cv=self.apply_cv,
                )
                spent_time = time.time() - start_time
                if temp_y == 0.0:
                    print("Invalid")
                ...
                pbar.set_description(
                    "Best val. acc.: %s" % str(np.round(best_val_acc, 4))
                )

                info_tuple = (
                    spent_time,
                    temp_y,
                    np.nanmax(response.detach().cpu().numpy()),
                    pipeline_id,
                )
                history_list.append(info_tuple)
        ...
        self.history_list = df = pd.DataFrame(
            history_list,
            columns=["Time", "ValidationPerformance", "Incumbent", "PipelineID"],
        )
        best_model_ix = int(
            df[df.Time < self.time_limit]["ValidationPerformance"].values.argmax()
        )
        ...
        if self.create_ensemble:
            self.ensemble = self.create_ensemble_from_models(
                fitted_pipelines, X_val, y_val
            )
            self.model = VotingClassifier(
                estimators=[(str(i), model) for i, model in enumerate(self.ensemble)],
                voting="hard",
            )
    ...
    def get_model(
        ...
        try:
            oboe_pipeline = pipeline.PipelineObject(
                p_type="classification", config=setting, index=pipeline_id, verbose=True
            )
    ...
    def get_pipelines_settings_on_dataset(
        self, settings, num_points_tr, num_features_tr
    ):
        def p2f(x):
        ...
        for item in settings:
            ...
            if item_copy["dim_reducer"]["algorithm"] == "PCA":
                item_copy["dim_reducer"]["hyperparameters"]["n_components"] = int(
                    min(
                        num_points_tr,
                        p2f(item_copy["dim_reducer"]["hyperparameters"]["n_components"])
                        * num_features_tr,
                    )
                )
            elif item_copy["dim_reducer"]["algorithm"] == "SelectKBest":
                item_copy["dim_reducer"]["hyperparameters"]["k"] = int(
                    p2f(item_copy["dim_reducer"]["hyperparameters"]["k"])
                    * num_features_tr
                )
