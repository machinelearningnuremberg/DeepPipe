        ...
        for alg in configs_single_step["algorithms"]:
            if alg is not None:
                ...
                for values in itertools.product(*hyperparams.values()):
                    ...
                    settings_step.append(
                        {"algorithm": alg, "hyperparameters": configs_single_alg}
                    )
            else:
...
def get_pipelines_settings_on_dataset(
    settings, n_folds, num_points_tr, num_features_tr
):
    def p2f(x):
    ...
    for item in settings:
        ...
        if item_copy["dim_reducer"]["algorithm"] == "PCA":
            item_copy["dim_reducer"]["hyperparameters"]["n_components"] = int(
                min(
                    (n_folds - 1) * num_points_tr / n_folds,
            ...
            item_copy["dim_reducer"]["hyperparameters"]["k"] = int(
                p2f(item_copy["dim_reducer"]["hyperparameters"]["k"]) * num_features_tr
            )
...
def generate_concat_matrix(list_for_concatenator, n_hidden_per_stage, device="cpu"):
    ...
    n_cols = sum(
        [n_hidden_per_stage[i] for i, stage in enumerate(list_for_concatenator)]
    )
    n_rows = sum(
        [
            n_hidden_per_stage[i]
    ...
    for i, stage in enumerate(list_for_concatenator):
        ...
        matrix[
            idx : idx + n_hidden_per_stage[i] * len(stage),
            idy : idy + n_hidden_per_stage[i],
        ] = temp_matrix
...
def get_matrix_and_masks(
    ...
    for i, stage in enumerate(list_for_concatenator):
        ...
        for j, algorithm in enumerate(stage):
            temp_dim_per_algorithm.append(
                algorithm_domains[idx + 1] - algorithm_domains[idx]
            )
            idx += 1
    ...
    hidden_size = [
        n_hidden_per_stage[i] for i in range(n_stages) for j in list_for_concatenator[i]
    ]
    ...
    if n_layers_encoder > 0:
        concat_matrix = generate_concat_matrix(
            list_for_concatenator, n_hidden_per_stage=n_hidden_per_stage, device=device
        )
    else:
        ...
        additional_hps = enc.fit_transform(list_for_selector.cpu()).toarray()[
            :, init_point:
        ]
...
def add_encoder_for_algorithms(
    algorithm_domains, list_for_selector, list_for_concatenator, new_encoder_input_dim
):
...
def get_scores(model, x, Lambda, y, eval_batch=1000):
    ...
    for i in range(eval_batch, c + eval_batch, eval_batch):
        ...
        score = EI(
            max(np.array(y.cpu())),
            predict_fn,
            support=np.where(np.array(x) < i)[0].tolist(),
            queries=Lambda,
