...
class MaskedMLP(nn.Module):
    def __init__(
        self,
        masks,
        list_for_concatenator,
        concat_matrix,
        hidden_dim,
        algorithm_domain,
        hidden_dim_per_stage=None,
        n_non_masked_layers=2,
        out_features=1,
        omit_estimator=-1,
        device="cpu",
    ):
        ...
        non_masked_layers.append(
            nn.Linear(self.hidden_dim_masked, hidden_dim).to(device)
        )
        for i in range(1, n_non_masked_layers - 1):
            non_masked_layers.append(nn.Linear(hidden_dim, hidden_dim).to(device))
        non_masked_layers.append(
            nn.Linear(hidden_dim, out_features).to(device)
        )  # aggregator layers

        self.masked_layers = nn.ModuleList(masked_layers)
        self.non_masked_layers = nn.ModuleList(non_masked_layers)
        self.select_masks = [
            torch.eye(len(i), device=device) for i in list_for_concatenator
        ]
        self.hidden_dim_per_stage = (
            [hidden_dim for _ in range(len(list_for_concatenator))]
            if hidden_dim_per_stage is None
            else hidden_dim_per_stage
        )
    ...
    def precompute_mask(self, select_mask_id):
        ...
        for i, mask in enumerate(self.select_masks):
            out.append(
                torch.kron(
                    mask[select_mask_id[:, i], :],
                    torch.ones(1, self.hidden_dim_per_stage[i], device=self.device),
                )
            )
                ...
                for i, mask in enumerate(self.select_masks):
                    out.append(
                        torch.kron(
                            mask[select_mask_id[:, i], :],
                            torch.ones(
                                1, self.hidden_dim_per_stage[i], device=self.device
                            ),
                        )
                    )
...
class Encoder(nn.Module):
    def __init__(self, n_input, hidden_dim, n_layers, device="cuda"):
        ...
        self.layers = nn.ModuleList(
            [
                nn.Linear(layer_size[i], layer_size[i + 1]).to(device)
...
class DeepPipe(nn.Module):
    def __init__(
        ...
        if self.debug:
            logging.basicConfig(
                filename=os.path.join(self.checkpoint_path, "log.txt"),
                level=logging.DEBUG,
            )
    ...
    def get_model_likelihood_mll(self, train_size):
        train_x = torch.ones(train_size, self.feature_extractor.out_features).to(
            self.device
        )
    ...
    def train(self, epoch, optimizer, verbose=False):
        ...
        for task in self.training_tasks:
            ...
            evaluated_pipelines = torch.where(
                ~torch.isnan(self.performance_matrix[task, :])
            )
            ...
            sampled_pipelines = np.random.choice(
                evaluated_pipelines[0].cpu().numpy(), batch_size
            )
        ...
        if self.debug:
            logging.info(
                self.train_metrics.report() + " " + self.valid_metrics.report()
            )
    ...
    def test(self, task):  # no optimizer needed for GP
        # (x_support, y_support),(x_query,y_query) = self.get_support_and_queries(task,train)

        evaluated_pipelines = (
            torch.where(~torch.isnan(self.performance_matrix[task, :]))[0].cpu().numpy()
        )
        support_idx = np.random.choice(
            evaluated_pipelines, replace=False, size=min(self.batch_size, 50)
        )
        query_idx = np.random.choice(
            np.setdiff1d(evaluated_pipelines, support_idx),
            replace=False,
    ...
    def __init__(self, train_x, train_y, likelihood, kernel, ard, nu, dims):
        ...
        if kernel == "rbf" or kernel == "RBF":
            self.covar_module = gpytorch.kernels.ScaleKernel(
                gpytorch.kernels.RBFKernel(ard_num_dims=dims if ard else None)
            )
        elif kernel == "matern":
            self.covar_module = gpytorch.kernels.ScaleKernel(
                gpytorch.kernels.MaternKernel(nu=nu, ard_num_dims=dims if ard else None)
            )
        ## Spectral kernel
        else:
            raise ValueError(
                "[ERROR] the kernel '"
                + str(kernel)
                + "' is not supported for regression, use 'rbf' or 'spectral'."
            )
...
class DeepKernelGP(nn.Module):
    ...
    def train(
        ...
        for epoch in range(epochs):
            ...
            if verbose:
                print(
                    "Iter {iter}/{epochs} - Loss: {loss:.5f}   noise: {noise:.5f} ".format(
                        iter=epoch + 1,
                        epochs=epochs,
                        loss=loss.item(),
        ...
        if self.debug:
            logging.info(
                f"Current Iteration: {len(support)} | Incumbent {max(self.Y[support])} | Duration {np.round(endtime-starttime,2)} | Epochs {epoch} | Noise {self.likelihood.noise.item()}"
            )
    ...
    def predict(self, support, query):
        x_support, y_support = self.X[support].to(self.device), self.Y[support].to(
            self.device
        )
...
class Metric(object):
    def __init__(self, prefix="train: ", n=16):
        if prefix == "train: ":
            self.message = (
                prefix + "loss: {loss:.2f} - noise: {log_var:.2f} - mse: {mse:.4f}"
            )
        else:
            self.message = (
                prefix
                + "loss spt.: {loss:.2f} - loss qry.: {log_var:.2f} - mse: {mse:.4f}"
            )
